...
export const CountriesWithinTimezones = () => {
	...
	const handleSubmit = () => {
		...
		if (start === -1 || end === -1) {
			setError('Please enter valid UTC timezones!');
			return;
		}

		const searchedRange = timezones.slice(
			Math.min(start, end),
		...
		const findNeighboringCountries = (country) => {
			if (
				...
				country.timezones.some((timezone) =>
					searchedRange.includes(timezone)
				)
			) {
				...
				if (country.borders?.length > 0) {
					country.borders.forEach((neighbour) => {
						const neighbouringCountry = data.find(
							(x) => x.cca3 === neighbour
						);
	...
	return (
		<div className={styles.container}>
			<div className={styles.inputDiv}>
				<label htmlFor="timezone-one">Enter timezone limit:</label>
				<input
					type="text"
					name="timezone-one"
					id="timezone-one"
			...
			<div className={styles.inputDiv}>
				<label htmlFor="timezone-two">Enter timezone limit:</label>
				<input
					type="text"
					name="timezone-two"
					id="timezone-two"
